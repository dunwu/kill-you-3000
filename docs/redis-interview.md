...
应用场景：适合用于存储结构化数据，如一个对象：用户信息、商品信息等。
...
适用场景：由于可以设置 score，且不重复。适合存储各种排行数据。
...
RDB 是将某个时间点的所有数据以快照方式写入磁盘。其实际操作是通过 fork 子进程执行，采用二进制压缩存储。可以将快照复制到其它服务器从而创建具有相同数据的服务器副本。
...
AOF 是以文本日志形式将所有写命令追加到 AOF 文件（Append Only File）的末尾。
...
- Redis 数据类型更丰富，支持  String、Hash、List、Set和Sorted Set ；Memcache 只支持 Key/Value。
- Redis支持数据备份，即 master-slave 模式的数据备份。
- Redis支持数据持久化，可以将内存中的数据保持在磁盘中，重启的时候可以再次加载进行使用；而Memecache把数据全部存在内存中。
- Redis的速度比Memcached快很多
- Redis使用单线程的IO复用模型。Memcached是多线程，非阻塞IO复用的网络模型。

## 缓存原理

Redis 常用于缓存，所以 Redis 相关的面试总是离不开缓存原理。

> :bulb: 请参考：[缓存基本原理](https://dunwu.github.io/blog/design/theory/cache-theory/ )
